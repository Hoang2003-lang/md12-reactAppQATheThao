      ...
      Alert.alert('Thành công', response.data.message, [
        {
          text: 'Đăng nhập',
          onPress: async () => {
            // Sau khi đăng ký thành công, tạo giỏ hàng cho người dùng
            try {
              const user = response.data.user; // Đảm bảo rằng bạn lấy được thông tin người dùng sau khi đăng ký thành công.
              const cartResponse = await API.post('/carts/add', {
                user_id: user._id,  // Gửi userId vào API để tạo giỏ hàng.
              });

              if (cartResponse.data.success) {
                Alert.alert('Giỏ hàng', 'Giỏ hàng đã được tạo thành công');
              } else {
                Alert.alert('Giỏ hàng', 'Tạo giỏ hàng thất bại');
              }

              navigation.navigate('Login'); // Điều hướng tới trang đăng nhập
            } catch (error) {
              Alert.alert('Lỗi', 'Tạo giỏ hàng thất bại');
            }
          },
        },
      ]);
      ...
      <View style={styles.container}>
        ...
        <View style={styles.checkboxContainer}>
          <TouchableOpacity onPress={() => setAgreeTerms(!agreeTerms)} style={styles.checkbox}>
            <View
              style={[styles.checkboxBox, agreeTerms && styles.checkboxChecked]}
            />
            <Text style={styles.checkboxText}>Tôi đồng ý</Text>
            <Text style={styles.checkboxText1}>
              {' '}
